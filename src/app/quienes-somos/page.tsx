...
import Navbar from '@/components/Navbar'; // Assuming you have a Navbar component, or I can inline it if preferred. I will inline a specialized version for this page to match the "Clean" request.
...
export default function QuienesSomos() {
  ...
  // DATA MOCKUP (To be animated in Phase 2)
  const stats = [
    { number: "12", label: "Años de Trayectoria" },
    { number: "+4", label: "Ciudades" },
    { number: "850+", label: "Familias Felices" },
    ...
    <div className="bg-[#F3F0EB] text-[#2C2C2C] antialiased min-h-screen flex flex-col font-sans selection:bg-[#C5A065] selection:text-white overflow-x-hidden">
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-4 max-w-5xl mx-auto pt-20">
            <span className="text-[#C5A065] font-bold uppercase tracking-[0.4em] text-xs mb-6 block animate-fade-in-up">
                INMAER Real Estate
            </span>
            <h1 className="font-serif-display text-6xl md:text-9xl text-[#2C2C2C] leading-[0.9] mb-8">
                Construimos <br/> <span className="italic font-light">Legado</span>
            </h1>
            <p className="text-[#484848] text-lg md:text-2xl font-light max-w-2xl mx-auto leading-relaxed">
                Más que desarrolladores, somos arquitectos de comunidades. <br/> Transformamos la tierra en el escenario de tu vida.
            </p>
        </div>
        
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 animate-bounce text-[#C5A065]">
          ...
          <div className="max-w-7xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-12 text-center">
              {stats.map((stat, i) => (
                  <div key={i} className="space-y-2">
                      <span className="font-serif-display text-5xl md:text-7xl text-[#C5A065] block">{stat.number}</span>
      ...
      <section className="py-24 px-6 bg-[#F9F7F4]">
          <div className="max-w-7xl mx-auto">
              ...
              <div className="grid md:grid-cols-4 gap-8">
                  {values.map((val, i) => (
                      <div key={i} className="bg-white p-10 shadow-sm border-t-2 border-[#C5A065] hover:-translate-y-2 transition-transform duration-500">
